...
import com.kerberos.travel.adapters.restaurantsAdapter;
...
public class RestaurantFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        ImageView back = view.findViewById(R.id.imageView34);

        back.setOnClickListener(v ->{
            requireActivity().getSupportFragmentManager().beginTransaction().replace(R.id.fragmentHolder,new HomeFragment()).commit();
        });

        DatabaseReference reference = FirebaseDatabase.getInstance().getReference().child("res");
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot childSnapshot : snapshot.getChildren()) {
                    RestaurantModel restaurantModel = childSnapshot.getValue(RestaurantModel.class);
                    restaurants.add(restaurantModel);
                }
                recyclerView.setLayoutManager(new LinearLayoutManager(requireActivity(),LinearLayoutManager.VERTICAL,false));
                recyclerView.setAdapter(new restaurantsAdapter(requireActivity(),restaurants));
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(requireContext(),error.getDetails(),Toast.LENGTH_LONG).show();
            }
        });
    }
}