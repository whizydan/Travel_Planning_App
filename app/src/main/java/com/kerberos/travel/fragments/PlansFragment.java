...
public class PlansFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(v -> {
            startActivity(new Intent(requireActivity(), AddPlanActivity.class));
        });

        DatabaseReference reference = FirebaseDatabase.getInstance().getReference().child("plans");
        reference.child(FirebaseAuth.getInstance().getCurrentUser().getUid().toString())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        for (DataSnapshot childSnapshot : snapshot.getChildren()) {
                            PlansModel plan = childSnapshot.getValue(PlansModel.class);
                            plans.add(plan);
                        ...
                        if(!plans.isEmpty()){
                            ...
                            adapter = new plansAdapter(requireActivity(),plans);
